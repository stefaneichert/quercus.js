...
| `showSelectAllButton` | `boolean`   | `false`     | If `true` and `multiSelectEnabled` is also `true` (and `nodeSelectionEnabled` is `true`), a "Select All" / "Deselect All" button will be displayed above the tree. |
...
| `nodeSelectionEnabled` | `boolean` | `true`      | If `true`, nodes can be selected by clicking their text. If `false`, node selection is disabled, and `onSelectionChange` will not be triggered by clicks on nodes. |
